...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`aw.datepicker.weeks.${ week }`)),
    dayNames: weeks.map(week => t(`aw.datepicker.weeks.${ week }`)),
    monthNamesShort: months.map(month => t(`aw.datepicker.months.${ month }`)),
    monthNames: months.map((month, index) => t(`aw.datepicker.month${ index + 1 }`)),
    amPm: ['am', 'pm']
  };
};
